"""
engine.py - the same multi-agent pipeline as RAG_Healthcare_Query_Assistant.ipynb,
packaged as an importable module for the Flask web app.

The functions here mirror the notebook one-to-one (same prompts, same 0.35 refusal
threshold, same read-only SQL guard). The only additions are:
  - lazy one-time build of the DB + FAISS index, and
  - answer_query(), which wraps ask_assistant() and also returns per-stage timings
    and structured evidence (SQL rows / retrieved sources) for the UI to display.

Nothing about the model logic differs from the notebook.
"""
...
# server-side conversation memory + audit log (same idea as notebook HISTORY / LOG)
HISTORY = []
LOG     = []

# ================================================================ LLM helper (notebook cell 4)
def _get_client():
    global _client
    if _client is None:
...
# ================================================================ data pipeline (notebook cells 8 & 10)
...
# ================================================================ policies + chunking (notebook cells 14 & 16)
POLICY_DOCUMENTS = {
"admission_policy": """
