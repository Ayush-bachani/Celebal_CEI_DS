"""
Flask server for the RAG-Based Healthcare Query Assistant.

It exposes the same pipeline that lives in RAG_Healthcare_Query_Assistant.ipynb
(imported from engine.py) over a small JSON API, and serves the MedQuery AI UI.
...
def chat():
    ...
    except Exception as e:  # never leak a 500 to the UI
